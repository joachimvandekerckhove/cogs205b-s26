"""Minimal Gemini REST client for `generateContent` (text, system instruction, attachments, structured JSON)."""
...
def build_system_instruction(
    *,
    text: str | None = None,
    skill_files: Sequence[Path | str] | None = None,
) -> dict[str, Any] | None:
    """
    Build a Gemini ``systemInstruction`` object (``parts`` with one text part).

    ``skill_files`` — paths to skill documents (e.g. ``SKILL.md``); each file is read as UTF-8.
    ``text`` — optional extra system text prepended before skill file contents.
    """
    chunks: list[str] = []
...
def _model_accepts_system_instruction(model: str) -> bool:
    """Gemma models return HTTP 500 when ``systemInstruction`` is sent; prepend to the user turn instead."""
...
class GeminiSimpleAPI:
    ...
    def generate_content_structured(
        ...
        *,
        ...
        ``system_instruction`` / ``skill_files`` — optional system prompt (see ``build_system_instruction``).
        On Gemma models the system text is prepended to the user prompt (API rejects ``systemInstruction``).
        """
        user_prompt = prompt
        sys_inst = build_system_instruction(text=system_instruction, skill_files=skill_files)
        if sys_inst is not None and not _model_accepts_system_instruction(self._model):
            user_prompt = f"{sys_inst['parts'][0]['text']}\n\n---\n\n{prompt}"
        parts = build_user_parts(user_prompt, attachments)
        body: dict[str, Any] = {
        ...
        if sys_inst is not None and _model_accepts_system_instruction(self._model):
    ...
    def prompt(
        ...
        *,
        ...
        content = self.generate_content_structured(
            prompt,
